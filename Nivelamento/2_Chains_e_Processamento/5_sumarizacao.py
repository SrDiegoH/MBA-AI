...
#Texto muito grande que precisa ser resumido seja para evitar o uso demasiado de tokens ou salvar em históricos (Módulo 4 - Gerenciamento de memória)
long_text = """
...
splitter = RecursiveCharacterTextSplitter(chunk_size=250, chunk_overlap=70) #Quebra o texto a cada 250 caracteres, mas permite que receba 70 caracteres da quebra anterior para não eprder o contexto
chunks = splitter.create_documents([long_text]) #Cria um documento quebrado
for chunk in chunks: #Mostra como estão as partes do documento
...
chain_sumarize = load_summarize_chain(model, chain_type="stuff", verbose=True) #Cria um chain para resumir dados. Mais detalhes sobre 'chain_type': https://bobrupakroy.medium.com/summarization-chain-vs-summarization-checker-chain-a-comprehensive-comparison-7afd7f873eb0

result = chain_sumarize.invoke({ "input_documents": chunks })
print(result) #Mostra no terminal o resultado do processo, neste caso Document resumido
print(f"Output: {result['output_text']}") #Saida final do Document